#include "MYOLED.h"
#include "dsp_soft_IIC.h"	/* 对接工程自己的软件 I2C 驱动(Bsp 目录,函数同名 MyI2C_*) */
#include "OLED_Data.h"
#include <string.h>
#include <math.h>
#include <stdio.h>
...
/*屏幕大小
  ...
  - 全屏 64 行 ÷ 8 = 8 页,页号 0~7
  - 页 0~1 = 蓝色区,页 2~7 = 黄色区(双色分界在这)
*/
...
/**
  ...
  */
  ...
  {
      /*使用STM32 HAL库的I2C通信函数，通过dsp_soft_IIC软件I2C模块向OLED写入命令*/
      MyI2C_WriteReg(OLED_ADDRESS, 0x00, Command);
  }
  ...
  void OLED_WriteData(uint8_t *Data, uint8_t Count)
  {
      ...
      /*使用STM32 HAL库的I2C通信函数，通过dsp_soft_IIC软件I2C模块向OLED写入数据*/
      MyI2C_Start();							//I2C起始
      MyI2C_SendByte(OLED_ADDRESS);			//发送OLED的I2C从机地址
      MyI2C_ReceiveAck();						//接收应答
      MyI2C_SendByte(0x40);					//控制字节，给0x40，表示即将写数据
      MyI2C_ReceiveAck();						//接收应答
      /*循环Count次，进行连续的数据写入*/
      for (i = 0; i < Count; i ++)
      {
          MyI2C_SendByte(Data[i]);				//依次发送Data的每一个数据
          MyI2C_ReceiveAck();					//接收应答
      }
      MyI2C_Stop();							//I2C终止
  }
  ...
  void OLED_Init(void)
  {
      /*I2C引脚初始化已在MyI2C_Init()中完成，无需在此重复初始化*/
  ...
  void OLED_ShowString(int16_t X, int16_t Y, char *String, uint8_t FontSize)
  {
      ...
      while (String[i] != '\0')	//遍历字符串
      {
          ...
          if (CharLength == 1)	//如果是单字节字符
          {
              ...
              XOffset += OLED_CHAR_WIDTH(FontSize);
          }
          else					//否则，即多字节字符
          {
              ...
              /*中文字符统一按16*16点阵显示(汉字没有6*8字模,直接查 OLED_CF16x16) */
  ...
  void OLED_ShowNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
  {
      ...
      for (i = 0; i < Length; i++)		//遍历数字的每一位
      {
          ...
          /*+ '0' 可将数字转换为字符格式*/
          OLED_ShowChar(X + i * OLED_CHAR_WIDTH(FontSize), Y, Number / OLED_Pow(10, Length - i - 1) % 10 + '0', FontSize);
      }
  }
  ...
  void OLED_ShowSignedNum(int16_t X, int16_t Y, int32_t Number, uint8_t Length, uint8_t FontSize)
  {
      ...
      for (i = 0; i < Length; i++)			//遍历数字的每一位
      {
          ...
          /*+ '0' 可将数字转换为字符格式*/
          OLED_ShowChar(X + (i + 1) * OLED_CHAR_WIDTH(FontSize), Y, Number1 / OLED_Pow(10, Length - i - 1) % 10 + '0', FontSize);
      }
  }
  ...
  void OLED_ShowHexNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
  {
      ...
      for (i = 0; i < Length; i++)		//遍历数字的每一位
      {
          ...
          if (SingleNumber < 10)			//单个数字小于10
          {
              ...
              /*+ '0' 可将数字转换为字符格式*/
              OLED_ShowChar(X + i * OLED_CHAR_WIDTH(FontSize), Y, SingleNumber + '0', FontSize);
          }
          else							//单个数字大于10
          {
              /*调用OLED_ShowChar函数，显示此数字*/
              /*+ 'A' 可将数字转换为从A开始的十六进制字符*/
              OLED_ShowChar(X + i * OLED_CHAR_WIDTH(FontSize), Y, SingleNumber - 10 + 'A', FontSize);
          }
      }
  }
  ...
  void OLED_ShowBinNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
  {
      ...
      for (i = 0; i < Length; i++)		//遍历数字的每一位
      {
          ...
          /*+ '0' 可将数字转换为字符格式*/
          OLED_ShowChar(X + i * OLED_CHAR_WIDTH(FontSize), Y, Number / OLED_Pow(2, Length - i - 1) % 2 + '0', FontSize);
      }
  }
  ...
  void OLED_ShowFloatNum(int16_t X, int16_t Y, double Number, uint8_t IntLength, uint8_t FraLength, uint8_t FontSize)
  {
      ...
      /*显示整数部分*/
      OLED_ShowNum(X + OLED_CHAR_WIDTH(FontSize), Y, IntNum, IntLength, FontSize);

      /*显示小数点*/
      OLED_ShowChar(X + (IntLength + 1) * OLED_CHAR_WIDTH(FontSize), Y, '.', FontSize);

      /*显示小数部分*/
      OLED_ShowNum(X + (IntLength + 2) * OLED_CHAR_WIDTH(FontSize), Y, FraNum, FraLength, FontSize);
  }
